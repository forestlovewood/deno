...
use fmt_errors::PrettyJsError;
use tools::test_runner;
use media_type::MediaType;
use module_graph::GraphBuilder;
use module_graph::Module;
use module_loader::CliModuleLoader;
use program_state::ProgramState;
use source_maps::apply_source_map;
use specifier_handler::FetchHandler;
...
pub async fn lsp_command() -> Result<(), AnyError> {
  lsp::start().await
}
...
pub async fn run_repl(flags: flags::Flags) -> Result<(), AnyError> {
...
pub async fn run_from_stdin(flags: flags::Flags) -> Result<(), AnyError> {
  let program_state = ProgramState::build(flags.clone()).await?;
  let permissions = Permissions::from_options(&flags.clone().into());
...
pub async fn run_with_watch(flags: flags::Flags, script: String) -> Result<(), AnyError> {
  let resolver = |_| {
...
pub async fn run_command(flags: flags::Flags, script: String) -> Result<(), AnyError> {
...
pub fn get_subcommand(
  ...
  match flags.clone().subcommand {
    ...
    flags::DenoSubcommand::Lsp => lsp_command().boxed_local(),
    flags::DenoSubcommand::Lint {
      files,
      rules,
      ignore,
      json,
    } => lint_command(flags, files, rules, ignore, json).boxed_local(),
    flags::DenoSubcommand::Repl => run_repl(flags).boxed_local(),
    flags::DenoSubcommand::Run { script } => run_command(flags, script).boxed_local(),
    flags::DenoSubcommand::Test {
      no_run,
      doc,
      fail_fast,
      quiet,
    ...
    .boxed_local(),
    flags::DenoSubcommand::Completions { buf } => {
